...
import axios from 'axios'; // OpenAI API 호출을 위한 axios 라이브러리 추가
...
const Evaluating = ({ selectedEmotion, taskDuration, selectedTask }) => {
  ...
  useEffect(() => {
    const fetchPomodoroCycle = async () => {
      try {
        const response = await axios.post(
          'https://api.openai.com/v1/engines/davinci-codex/completions', // 적절한 엔드포인트로 변경
          {
            prompt: `Emotion: ${selectedEmotion}, Task Duration: ${taskDuration}, Task Name: ${selectedTask.task_name}. Calculate the optimal Pomodoro timer pattern.`,
            max_tokens: 100,
          },
          {
            headers: {
        ...
        const [focusTime, breakTime, cycles] = result.split(',').map(Number);
    ...
    setTimeout(fetchPomodoroCycle, 2000); // API 호출을 2초 후에 수행 (로딩 화면을 위해)
  }, [selectedEmotion, taskDuration, selectedTask]);
